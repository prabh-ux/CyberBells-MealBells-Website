...
import { fetchVendorList, fetchVendorKpi } from "../../slices/vendorPerformanceSlice";
...
const VendorPerformance = () => {
  ...
  const { vendors, listLoading, kpiCache, kpiLoading } =
    useSelector((s: RootState) => s.vendorPerformance);
  ...
  // Always fetch when vendorId or period changes — no cache gate here.
  // The slice still caches results so going back to a previous selection
  // will show the cached value instantly while the fresh fetch completes.
  useEffect(() => {
    dispatch(fetchVendorKpi({ vendorId, period }));
    setSelectedDay(null);
    setShowAll(false);
  }, [vendorId, period, dispatch]);

  const cacheKey = `${vendorId}__${period}`;
  const kpi      = kpiCache[cacheKey] ?? null;

  // Show loading skeleton only when there is no cached data yet for this key.
      ...
      <div className="mx-auto w-full px-6 py-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* Subtle refresh indicator when re-fetching over cached data */}
            {kpiLoading && kpi && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* FIX #3 — was using a hardcoded [starFull×4, starHalf×1] array that
              always showed 4.5 stars regardless of kpi.rating. Now uses the
              same StarRating component used in the feedback table. */}
          <KpiCard
