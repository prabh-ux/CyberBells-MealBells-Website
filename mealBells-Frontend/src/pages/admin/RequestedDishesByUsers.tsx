...
const VendorModal: React.FC<VendorModalProps> = ({ request, vendors, loading, onClose, onForward }) => {
  ...
  const selectAll   = () => setSelected(allSelected ? new Set() : new Set(vendors.map(v => v._id)));

  return (
    // Full-screen overlay — safe on all screen sizes
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md p-5 sm:p-6 max-h-[90vh] flex flex-col">
        ...
        <div className="space-y-2 overflow-y-auto flex-1 pr-1 mb-4">
          {vendors.length === 0 ? (
            ...
            return (
              <button key={v._id} onClick={() => toggle(v._id)}
                className={`w-full flex items-center gap-3 p-3 rounded-2xl border transition-all text-left ${
                  checked ? "bg-orange-50 border-orange-300 shadow-sm" : "bg-gray-50 border-gray-200 hover:border-orange-200"
                }`}>
...
const RequestCard: React.FC<CardProps> = ({ request, vendors, forwarding, onForward }) => {
  ...
  return (
    <>
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-4 sm:p-5 flex flex-col gap-3 sm:gap-4">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0 ${avatarColor(user?.name ?? "U")}`}>
              {user?.avatar
            ...
            <div className="min-w-0">
              ...
              {user?.department && <p className="text-[11px] text-gray-400 truncate">{user.department}</p>}
            </div>
          </div>
              ...
              {forwardedTo.map((f) => {
                ...
                return (
                  <span key={key}
                    ...
                    {f.vendorStatus !== "pending" && <span className="opacity-70 capitalize">· {f.vendorStatus}</span>}
                  </span>
                );
              })}
            </div>
        ...
        {/* Forward button */}
        <button onClick={() => setModalOpen(true)} disabled={isForwarding}
      ...
      {modalOpen && (
        <VendorModal
          request={request} vendors={vendors} loading={isForwarding}
          onClose={() => setModalOpen(false)}
          onForward={(reqId, vendorIds) => { onForward(reqId, vendorIds); setModalOpen(false); }}
        />
      )}
    </>
  );
...
const RequestedDishesByUsers: React.FC = () => {
  ...
  useEffect(() => { if (vendors.length === 0) dispatch(fetchVendors()); }, [dispatch]);
  useEffect(() => { dispatch(fetchAdminDishRequests({ status: statusFilter })); }, [dispatch, statusFilter]);
  ...
  return (
    <div className="min-h-screen bg-[#fdfcfa] py-6 sm:py-10 px-3 sm:px-6">
      <div className="w-full max-w-7xl mx-auto">
        ...
        {/* Header — always a row, title on left, button on right */}
        <div className="flex flex-row items-center justify-between mb-5 sm:mb-8 gap-3">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-3xl font-bold text-gray-900 leading-tight truncate">Dish Requests</h1>
            <p className="text-gray-400 text-[11px] sm:text-sm mt-0.5 hidden xs:block">
          ...
          <button onClick={refresh}
            ...
            <RefreshCw size={13} strokeWidth={2} className={adminLoading ? "animate-spin" : ""} />
            Refresh
          </button>
        </div>
...
{/* Stats — 3 cols always, shrink text on mobile */}
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6 sm:mb-8">
          {stats.map(({ label, value, Icon, cls }) => (
            <div key={label} className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 shadow-sm p-2 sm:p-5 flex flex-col sm:flex-row items-center sm:items-center gap-1 sm:gap-4">
        ...
        {adminLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 sm:py-20 gap-3">
            ...
            <p className="text-sm font-semibold text-gray-500 text-center px-4">{adminError}</p>
            <button onClick={refresh} className="text-xs text-orange-500 font-semibold hover:underline">Try again</button>
          </div>
