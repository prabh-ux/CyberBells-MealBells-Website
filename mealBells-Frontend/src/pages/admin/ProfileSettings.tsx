...
const ROLES = ["Standard User", "Department Head", "System Admin"] as const;
                    ...
                    <div className="absolute z-20 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
                      {ROLES.map(r => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 text-sm hover:bg-[#FFF4EC] hover:text-[#FA7000] transition-colors
                            ${role === r ? "bg-[#FFF4EC] text-[#FA7000] font-semibold" : "text-slate-700"}`}
                        >
                          {r}
                        </button>
                      ))}
