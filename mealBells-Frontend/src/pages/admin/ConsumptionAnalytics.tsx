...
export default function ConsumptionAnalytics() {
  // डोनट चार्ट और बार चार्ट के होवर स्टेट्स
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <select className="border border-gray-200 rounded-lg px-3 py-1.5 text-xs font-medium text-orange-600 bg-white shadow-sm focus:outline-none cursor-pointer">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
            <div className="flex items-center justify-between mb-5">
              ...
              <div className="flex items-center gap-4 text-xs font-medium text-gray-500">
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#934411]"></span>
                  Veg
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-full border-2 border-dashed border-[#4a3e3d]"></span>
                  Non-Veg
                </div>
              </div>
            </div>
            ...
            <div className="relative h-64 w-full overflow-hidden mt-8">
              <svg viewBox="0 0 700 220" className="w-full h-full">
                <path
                  d="M 20 160 Q 150 110, 250 120 T 480 150 T 620 70 T 680 90"
                  fill="none"
                  stroke="#4a3e3d"
                  strokeWidth="2.5"
                  strokeDasharray="5 5"
                ...
                />
                <path
                  d="M 20 180 Q 150 130, 250 140 T 480 170 T 620 90 T 680 110"
                  fill="none"
                  stroke="#934411"
                  strokeWidth="2.5"
                  strokeLinecap="round"
              ...
              <div className="relative w-44 h-44 flex items-center justify-center">
                ...
                <svg viewBox="0 0 42 42" className="w-full h-full transform -rotate-90 select-none outline-none border-none">
                  <circle
                    ...
                    className="cursor-pointer transition-all duration-200 hover:stroke-gray-300 outline-none focus:outline-none active:outline-none"
                    style={{ outline: 'none', WebkitTapHighlightColor: 'transparent' }}
                    onMouseEnter={() => setHoveredSlice("vegan")}
                  ...
                  <circle
                    ...
                    className="cursor-pointer transition-all duration-200 hover:opacity-90 outline-none focus:outline-none active:outline-none"
                    style={{ outline: 'none', WebkitTapHighlightColor: 'transparent' }}
                ...
                <div className="absolute w-[104px] h-[104px] bg-white rounded-full flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider transition-all duration-150">
                    {centerText.label}
                  </p>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] overflow-hidden">
            ...
            <div className="w-full overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              <div className="min-w-[340px]">
                <div className="grid grid-cols-8 gap-2 mb-3 text-center">
                  <div></div>
                  {daysOfWeek.map((day) => (
                    <span key={day} className="text-[11px] font-medium text-gray-400">
                      {day}
                    </span>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {heatmapRows.map((row) => (
                    <div key={row.name} className="grid grid-cols-8 gap-2 items-center">
                      <span className="text-xs font-medium text-gray-500 text-left pr-2 truncate">
                        {row.name}
                      </span>
                      {row.data.map((bgClass, idx) => (
                        <div
                          key={idx}
                          className={`aspect-square rounded-md ${bgClass} transition-all duration-200 hover:scale-105`}
                        ></div>
                      ))}
                    </div>
                  ))}
            ...
            <div className="flex justify-end items-center gap-2 mt-6 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              <span>Less</span>
              <div className="w-3 h-3 rounded bg-[#fff7ed]"></div>
              <div className="w-3 h-3 rounded bg-[#fff2e6]"></div>
              <div className="w-3 h-3 rounded bg-[#fed7aa]"></div>
              <div className="w-3 h-3 rounded bg-[#ffb066]"></div>
              <div className="w-3 h-3 rounded bg-[#ff7a00]"></div>
              <span>More</span>
            </div>
          </div>

          {/* Bar Chart (नया अपडेटेड अटेंडेंस स्टाइल टूलटिप) */}
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
            ...
            <div className="flex items-end justify-between h-48 gap-3 px-2 relative">
              {barHeights.map((height, index) => {
                // टूलटिप की पोजीशन को सीधे बार की हाइट के अनुसार सेट करने के लिए कैलकुलेशन
                const tooltipBottom = height + 8; 
                ...
                return (
                  ...
                  >
                    {/* इमेज की तरह दिखने वाला फ्लोटिंग अटेंडेंस स्टाइल कार्ड */}
                    <div
            ...
            <div className="flex justify-between mt-4 text-[11px] font-medium text-gray-400 px-1">
              {daysOfWeek.map((day) => (
                <span key={day} className="w-full text-center flex-1">{day}</span>
              ))}
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-black-400 font-bold text-[14px] uppercase tracking-wider">
                <tr>
                  ...
                  <th className="text-left px-6 py-4">Item Ordered</th>
