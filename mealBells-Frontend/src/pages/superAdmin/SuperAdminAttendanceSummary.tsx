...
import {
  ...
  ChevronDown, ChevronUp, MoreHorizontal, Building2,
} from "lucide-react";
...
function DonutChart({
  ...
  return (
    <svg viewBox="0 0 190 190" className="w-full h-full" style={{ overflow: "visible" }}>
      <defs>
        <filter id="super-seg-shadow-orange" x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="2" stdDeviation="5" floodColor="#f97316" floodOpacity="0.4" />
        </filter>
        <filter id="super-seg-shadow-gray" x="-20%" y="-20%" width="140%" height="140%">
      ...
      <circle cx={cx} cy={cy} r={r} fill="none"
        ...
        style={{ cursor: "pointer", transition: "stroke-width 0.2s, stroke 0.2s", filter: isHA ? "url(#super-seg-shadow-gray)" : "none" }}
      ...
      <circle cx={cx} cy={cy} r={r} fill="none"
        ...
        strokeDashoffset={0}
        strokeLinecap="butt" transform={`rotate(-90 ${cx} ${cy})`}
        style={{ cursor: "pointer", transition: "stroke-width 0.2s, stroke 0.2s", filter: isHP ? "url(#super-seg-shadow-orange)" : "none" }}
...
export default function SuperAdminAttendanceSummary() {
  ...
  } = useSelector((state: RootState) => state.superAnalytics);
  ...
  // Vendor options depend on the active org — refetch whenever it changes
  useEffect(() => { dispatch(fetchSuperFilterOptions(activeOrgId)); }, [dispatch, activeOrgId]);

  useEffect(() => {
    dispatch(fetchSuperAnalyticsSummary(filters));
    dispatch(fetchSuperMealsChart(filters));
    dispatch(fetchSuperAttendanceChart(filters));
    setTablePage(0);
  }, [filters]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleDateRange  = (label: string) => dispatch(setSuperFilters({ days: DATE_LABEL_TO_DAYS[label] }));
  const handleDepartment = (val: string)   => dispatch(setSuperFilters({ department: val === "All Departments" ? "all" : val }));
  const handleMealType   = (val: string)   => dispatch(setSuperFilters({ mealType: MEAL_TYPE_TO_FILTER[val] ?? val }));
  const handleClear      = ()              => dispatch(resetSuperFilters());

  const key            = superCacheKey(filters);
    ...
    <div className="min-h-screen bg-[#F8F9FB] p-3 sm:p-5 lg:p-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-400 mt-0.5 flex items-center gap-1.5">
            {activeOrgId !== "all" && <Building2 size={12} className="text-orange-400 shrink-0" />}
      ...
      {activeOrgId === "all" && (
        <div className="flex items-center gap-2 mb-4 sm:mb-6 px-4 py-3 rounded-2xl bg-amber-50 border border-amber-100 text-amber-700 text-sm">
          ...
          Showing combined attendance across all organizations. Select an organization from the header to drill in.
        </div>
      )}
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-3 sm:gap-4 items-end">
            ...
            <div className="flex flex-col gap-1">
              ...
              <DropDown
                ...
                onChange={(label) => {
                  if (label === "All Vendors") { dispatch(setSuperFilters({ vendorId: "all" })); return; }
                  const match = vendorOptions.find(v => v.label === label);
                  if (match) dispatch(setSuperFilters({ vendorId: match.value }));
                }}
              />
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <StatCard icon={<UtensilsCrossed size={16} />} label={`Meals (Today)`} accent="#f97316"
