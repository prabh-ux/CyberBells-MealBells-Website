...
const buildSvgPath = (points: number[], svgW = 700, svgH = 200, padX = 20, padY = 20): string => {
...
export default function SuperAdminConsumptionPage() {
  ...
  // Redux
  const { orgOptions, filters: analyticsFilters, mealsChart, summary, summaryLoading } =
  ...
  useEffect(() => {
    ...
    dispatch(fetchSuperConsumptionBreakdown({ days, orgId: activeOrgId, department: "all", vendorId: "all", mealType: "all" }));
  }, [days, activeOrgId]); // eslint-disable-line

  useEffect(() => {
    dispatch(fetchSuperLiveFeed({ limit: 20, orgId: activeOrgId }));
    const id = setInterval(() => dispatch(fetchSuperLiveFeed({ limit: 20, orgId: activeOrgId })), 30_000);
  ...
  const barHeights = last7.map(d => Math.max(4, Math.round((d.count / maxCount) * BAR_MAX_H)));
  ...
  const donutCenter = useMemo(() => {
    ...
    if (hoveredSlice === "both")   return { count: (mb.bothCount   ?? 0).toLocaleString(), label: "Both" };
  ...
  const stats = [
    {
      title: "Total Consumption", icon: <Utensils className="w-5 h-5 text-[#934411]" />, bg: "bg-[#fdf2ec]",
      value: isLoading ? "…" : `${totalConsumed.toLocaleString()} Meals`,
      badge: summary?.userGrowthPct != null ? `${summary.userGrowthPct > 0 ? "+" : ""}${summary.userGrowthPct}%${summary.userGrowthPct >= 0 ? "↑" : "↓"}` : "—",
      badgeColor: (summary?.userGrowthPct ?? 0) >= 0 ? "text-[#c25e1a]" : "text-[#b91c1c]",
    },
    {
      title: "Highest Demand Dish", icon: <Flame className="w-5 h-5 text-[#3b82f6]" />, bg: "bg-[#eff6ff]",
      value: isLoading ? "…" : breakdown?.topDish?.name ?? "—",
      badge: isLoading ? "—" : `Popularity: ${breakdown?.topDish?.popularity ?? 0}%`,
      badgeColor: "text-slate-500",
    },
    {
      title: "Most Active Dept", icon: <Users2 className="w-5 h-5 text-[#0284c7]" />, bg: "bg-[#f0f9ff]",
      value: isLoading ? "…" : breakdown?.mostActiveDept?.name ?? "—",
      badge: isLoading ? "—" : `${(breakdown?.mostActiveDept?.count ?? 0).toLocaleString()} Meals`,
      badgeColor: "text-[#0284c7]",
    },
    {
      title: "Least Active Day", icon: <CalendarDays className="w-5 h-5 text-[#b91c1c]" />, bg: "bg-[#fef2f2]",
      value: isLoading ? "…" : breakdown?.leastActiveDay?.name ?? "—",
      badge: "Fewest meals", badgeColor: "text-[#b91c1c]",
    },
  ];
    ...
    <div className="min-h-screen bg-[#fcfcfc] p-4 md:p-6 select-none overflow-y-auto">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 py-1.5">
              ...
              <select
                value={timeFrame}
                onChange={e => setTimeFrame(e.target.value as typeof TIME_FRAMES[number])}
                className="text-xs font-semibold text-gray-700 bg-transparent outline-none cursor-pointer"
              >
                {TIME_FRAMES.map(t => <option key={t}>{t}</option>)}
              </select>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {stats.map((s, i) => (
            <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:scale-[1.01] transition-transform">
              <div className="flex items-center justify-between mb-5">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.bg}`}>{s.icon}</div>
                <span className={`text-[11px] font-bold ${s.badgeColor}`}>{s.badge}</span>
              </div>
              <p className="text-[12px] font-semibold text-[#8a5d45] mb-2">{s.title}</p>
              {isLoading
                ? <div className="h-6 w-32 bg-gray-100 animate-pulse rounded" />
                : <h3 className="text-[20px] font-bold text-gray-900 leading-none">{s.value}</h3>
              }
            </div>
          ))}
        </div>
            ...
            {chartData.length === 0 ? (
              ...
              <div className="relative h-64 w-full overflow-hidden mt-8">
                <svg viewBox="0 0 700 220" className="w-full h-full">
                  {[55, 110, 165].map(y => (
                    <line key={y} x1="20" y1={y} x2="680" y2={y} stroke="#f1f5f9" strokeWidth="1" />
                  ))}
                  {linePath && <path d={linePath} fill="none" stroke="#934411" strokeWidth="2.5" strokeLinecap="round" />}
                  {chartData.map((d, i) => {
                    const max = Math.max(...chartData.map(x => x.count), 1);
                    const x   = 20 + (i / Math.max(chartData.length - 1, 1)) * 660;
                    const y   = 200 - 20 - (d.count / max) * 160;
                    return <circle key={i} cx={x} cy={y} r="3" fill="white" stroke="#934411" strokeWidth="2" />;
                  })}
                </svg>
              ...
              <>
                <div className="flex justify-center items-center flex-1">
                  <div className="relative w-44 h-44 flex items-center justify-center">
                    <svg viewBox="0 0 42 42" className="w-full h-full transform -rotate-90 outline-none">
                      {(vegPct + nonVegPct + bothPct === 0) ? (
                        <circle cx="21" cy="21" r="15.915" fill="none" stroke="#e2e8f0" strokeWidth="5.8" strokeDasharray="100 0" />
                      ) : (
                        <>
                          <circle cx="21" cy="21" r="15.915" fill="none" stroke="#e2e8f0" strokeWidth="5.8"
                            strokeDasharray={`${bothPct} ${100 - bothPct}`} strokeDashoffset="0"
                            className="cursor-pointer"
                            onMouseEnter={() => setHoveredSlice("both")} onMouseLeave={() => setHoveredSlice(null)} />
                          <circle cx="21" cy="21" r="15.915" fill="none" stroke="#934411" strokeWidth="5.8"
                            strokeDasharray={`${vegPct} ${100 - vegPct}`} strokeDashoffset={`-${bothPct}`}
                            className="cursor-pointer"
                          ...
                          <circle cx="21" cy="21" r="15.915" fill="none" stroke="#ff7a00" strokeWidth="5.8"
                            strokeDasharray={`${nonVegPct} ${100 - nonVegPct}`} strokeDashoffset={`-${bothPct + vegPct}`}
                            className="cursor-pointer"
                    ...
                    <div className="absolute w-[104px] h-[104px] bg-white rounded-full flex flex-col items-center justify-center pointer-events-none">
                      <h3 className="text-2xl font-extrabold text-gray-800">{donutCenter.count}</h3>
                      <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">{donutCenter.label}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {heatmapRows.map(row => (
                      <div key={row.dept} className="grid grid-cols-8 gap-2 items-center">
                        <span className="text-xs font-medium text-gray-500 truncate pr-2" title={row.dept}>{row.dept}</span>
                        {row.counts.map((count, idx) => (
                          <div key={idx} title={`${count} meals`}
                ...
                <div className="flex items-end justify-between h-48 gap-3 px-2 relative">
                  {last7.map((d, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center justify-end h-full relative"
                      ...
                      <div className={`absolute bg-white border border-orange-100 rounded-xl p-2 shadow-md flex flex-col text-left min-w-[80px] transition-all duration-200 z-10 ${
                        hoveredBar === i ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
                      }`} style={{ bottom: `${barHeights[i] + 8}px` }}>
                        <span className="text-[11px] font-bold text-gray-800">{d.day}</span>
                        <span className="text-[11px] font-bold text-red-950 whitespace-nowrap mt-0.5">{d.count.toLocaleString()} meals</span>
                        <span className="text-[10px] text-gray-400">{d.fullDate}</span>
                      </div>
                      <div className="w-full bg-[#6e3d10] rounded-t-lg transition-all duration-300"
                        style={{ height: `${barHeights[i]}px` }} />
                    </div>
                  ))}
                </div>
                <div className="flex justify-between mt-4 text-[11px] text-gray-400 px-1">
                  {last7.map((d, i) => <span key={i} className="w-full text-center flex-1">{d.day}</span>)}
                </div>
              </>
            )}
          </div>
          ...
          <div className="p-5 border-b border-gray-50 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {activeOrgId === "all" ? "Across all organizations" : `From: ${activeOrgLabel}`}
              </p>
            </div>
