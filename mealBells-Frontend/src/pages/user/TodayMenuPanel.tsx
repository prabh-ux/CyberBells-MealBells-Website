...
export default function TodayMenuPanel() {
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f5] gap-4 px-4">
        <UtensilsCrossed className="w-16 h-16 text-gray-300" />
        <p className="text-gray-400 text-lg font-medium text-center">No menu scheduled for today</p>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto lg:max-w-none">
        ...
        <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <div className="flex items-start gap-3 sm:gap-5">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2 sm:mb-3">
                  <span className={`w-4 h-4 sm:w-5 sm:h-5 rounded-sm border-2 flex items-center justify-center flex-shrink-0 ${
                ...
                <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 leading-snug mb-1.5 sm:mb-2">
                  {dish.name}
                </h2>
                <p className="text-sm text-gray-400 leading-relaxed line-clamp-2 mb-4 sm:mb-5">
                  {dish.description}
                </p>
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  {colleaguesEating > 0 ? (
                      ...
                      <div className="flex items-center">
                        {visibleAvatars.length > 0
                          ? visibleAvatars.map((src, i) => (
                              <div key={i}
                                className="w-7 h-7 sm:w-9 sm:h-9 rounded-full border-2 border-white overflow-hidden flex-shrink-0"
                                style={{ zIndex: 10 - i, marginLeft: i === 0 ? 0 : -8 }}>
                                <img src={src} alt="" className="w-full h-full object-cover"
                                  onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                              </div>
                            ))
                          : <InitialsAvatar size={32} />
                        }
                        {overflowCount > 0 && (
                          <div
                            className="w-7 h-7 sm:w-9 sm:h-9 rounded-full border-2 border-white bg-orange-50 flex items-center justify-center text-orange-500 text-[10px] sm:text-xs font-bold flex-shrink-0"
                            style={{ marginLeft: visibleAvatars.length > 0 ? -8 : 0 }}>
                            +{overflowCount}
                          </div>
                        )}
                      </div>
                    ...
                    <span className="text-xs sm:text-sm text-gray-400">No colleagues eating yet</span>
                  )}
                </div>
              </div>

              {dish.image ? (
                <div className="w-24 h-24 sm:w-32 sm:h-32 lg:w-36 lg:h-36 rounded-xl sm:rounded-2xl overflow-hidden flex-shrink-0">
                  <img src={dish.image} alt={dish.name} className="w-full h-full object-cover" />
                </div>
              ) : (
                <div className="w-24 h-24 sm:w-32 sm:h-32 lg:w-36 lg:h-36 rounded-xl sm:rounded-2xl flex-shrink-0 bg-gray-100 flex items-center justify-center">
                  <UtensilsCrossed className="w-8 h-8 sm:w-10 sm:h-10 text-gray-300" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-sm border border-gray-100">
            <p className="text-lg sm:text-xl font-semibold text-gray-800 text-center mb-5 sm:mb-6">
              ...
              <>
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  ...
                  <button
                    ...
                    className={`py-6 sm:py-8 rounded-2xl font-bold text-lg sm:text-xl transition-all duration-200 disabled:opacity-60 flex flex-col items-center justify-center gap-2 sm:gap-3 border-2 ${
                      myResponse === "yes"
                  ...
                  >
                    <span className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-colors duration-200 ${
                      myResponse === "yes" ? "bg-white/30 ring-2 ring-white/60" : "bg-white/20"
                    }`}>
                      <Check className={`w-5 h-5 sm:w-6 sm:h-6 stroke-[3] ${myResponse === "no" ? "text-gray-300" : "text-white"}`} />
                    </span>
                    Yes
                  </button>
                  ...
                  >
                    <span className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-colors duration-200 ${
                      ...
                      <X className={`w-5 h-5 sm:w-6 sm:h-6 stroke-[3] ${myResponse === "yes" ? "text-gray-300" : "text-white"}`} />
                    </span>
                    No
                  </button>
                </div>
                ...
                {myResponse && (
                  <div className={`mt-4 sm:mt-5 text-center text-sm sm:text-base font-semibold py-3 sm:py-4 rounded-2xl flex items-center justify-center gap-2 ${
                    myResponse === "yes" ? "bg-green-50 text-green-600" : "bg-red-50 text-red-500"
                  }`}>
                    {myResponse === "yes"
                      ? <><Check className="w-4 h-4 sm:w-5 sm:h-5 stroke-[3]" /> You're in for today!</>
                      : <><X className="w-4 h-4 sm:w-5 sm:h-5 stroke-[3]" /> See you next time!</>
                    }
                  </div>
                )}
