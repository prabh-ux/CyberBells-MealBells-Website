...
import {
  ChevronLeft, Flame, UtensilsCrossed, Star, Package,
  Gem, Leaf, Smile, Meh, Frown, PartyPopper, Pencil, Send,
} from "lucide-react";
import type { AppDispatch, RootState } from "../../app/store";
import { fetchTodayMenu, fetchMyReviews, submitReview } from "../../slices/userSlice";

interface Scores { taste: number; quantity: number; quality: number; freshness: number; }

const CRITERIA: { key: keyof Scores; label: string; Icon: React.FC<{ className?: string }> }[] = [
...
const TAGS = [
  "Perfectly Seasoned", "Too Salty", "Great Portion", "Cold Food",
  "Loved It", "Need More Variety", "Fresh Ingredients", "Overcooked",
];

const RATING_META: Record<number, { label: string; Icon: React.FC<{ className?: string }> }> = {
  ...
  return (
    <div className="flex gap-2">
      {[1, 2, 3, 4, 5].map((s) => (
        ...
        >
          <Star className={`w-10 h-10 transition-colors ${s <= (hover || value) ? "fill-yellow-400 text-yellow-400" : "fill-gray-100 text-gray-200"}`} />
        </button>
      ))}
    </div>
  );
...
function SliderRating({ value, onChange, Icon, label }: {
...
export default function RateMealPanel() {
  ...
  const loading    = menuLoading || reviewsLoading;
  const fetchError = !menu && !menuLoading ? "No meal scheduled for today." : "";

  // local form state
  const [overallRating, setOverallRating] = useState(0);
  const [scores, setScores]               = useState<Scores>({ taste: 3, quantity: 3, quality: 3, freshness: 3 });
  const [comment, setComment]             = useState("");
  const [selectedTags, setSelectedTags]   = useState<string[]>([]);
  const [isEditing, setIsEditing]         = useState(false);
  const [submitted, setSubmitted]         = useState(false);

  // Fetch today's menu + reviews on mount
  useEffect(() => {
    dispatch(fetchTodayMenu());
    dispatch(fetchMyReviews({ page: 1, limit: 50 }));
  }, [dispatch]);

  // When reviews arrive, prefill if already reviewed
  const existingReview = reviewsData?.reviews.find((r) => r.scheduleId === menu?.scheduleId);
  ...
  useEffect(() => {
    if (existingReview) {
      ...
      setScores({
        ...
        quantity:  existingReview.quantity,
        quality:   existingReview.quality,
        freshness: existingReview.freshness,
      });
  ...
  const handleSubmit = async () => {
    ...
    const result = await dispatch(submitReview({
      scheduleId: menu.scheduleId, overallRating, ...scores, comment, tags: selectedTags,
    }));
  ...
  if (fetchError || !menu) return (
    <div className="min-h-screen bg-[#F7F6F3] flex flex-col items-center justify-center gap-4 px-8 text-center">
      ...
      <p className="text-gray-500 text-sm">{fetchError || "No meal found for today."}</p>
  ...
  if (submitted) return (
    <div className="min-h-screen bg-[#F7F6F3] flex flex-col items-center justify-center px-8 text-center">
      ...
      <p className="text-gray-400 mb-6">Your {overallRating}-star review helps us serve you better.</p>
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3]">
      <div className="max-w-7xl mx-auto px-4 py-8 lg:py-12">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 leading-tight">Rate Your Meal</h1>
        </div>
        ...
        {hasExistingReview && !isEditing && (
          <div className="mb-6 flex items-center justify-between bg-orange-50 border border-orange-200 rounded-2xl px-5 py-3.5">
            <div className="flex items-center gap-2.5">
              ...
              <p className="text-sm font-semibold text-orange-700">You've already reviewed this meal.</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-5">
            <div className="relative rounded-[28px] overflow-hidden h-72 lg:h-96 shadow-[0_4px_24px_rgba(0,0,0,0.10)]">
              ...
              <div className="absolute top-4 left-4">
                <span className={`text-[11px] font-bold uppercase tracking-wide px-3 py-1.5 rounded-full flex items-center gap-1.5 bg-white ${dish.dishType === "Veg" ? "text-green-600" : "text-red-500"}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${dish.dishType === "Veg" ? "bg-green-500" : "bg-red-500"}`} />
                  {dish.dishType === "Veg" ? "Vegetarian" : "Non-Veg"}
                </span>
              </div>
              <div className="absolute bottom-5 left-5 right-5">
                <p className="text-orange-300 text-xs font-bold uppercase tracking-widest mb-1">Today's Meal</p>
                <h1 className="text-white text-2xl font-bold leading-tight">{dish.name}</h1>
              </div>
            </div>
            ...
            {(dish.estimatedCalories || dish.tags?.length) && (
              <div className="flex flex-wrap gap-2">
                ...
                {dish.tags?.slice(0, 3).map((t) => (
                  <span key={t} className="bg-orange-50 text-orange-600 text-[12px] font-semibold px-3 py-1.5 rounded-full border border-orange-100">{t}</span>
                ))}
              </div>
            )}

            <div className={`bg-white rounded-[24px] p-6 shadow-sm border text-center transition-all ${!formActive ? "border-gray-100 opacity-70 pointer-events-none select-none" : "border-gray-100"}`}>
              <p className="text-gray-400 text-sm font-medium mb-4">How was your overall experience?</p>
              <div className="flex justify-center mb-3">
                <StarRating value={overallRating} onChange={formActive ? setOverallRating : () => {}} />
              </div>
              {overallRating === 0
          ...
          {/* RIGHT: Review form */}
          <div className="space-y-5">
            <div className={`bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 transition-all ${!formActive ? "opacity-70 pointer-events-none select-none" : ""}`}>
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-5">Rate Each Aspect</h3>
              <div className="space-y-5">
                {CRITERIA.map((c) => (
                  <SliderRating key={c.key} label={c.label} Icon={c.Icon}
                    value={scores[c.key]} onChange={(v) => setScores((s) => ({ ...s, [c.key]: v }))}
                  />
                ))}
              </div>
            </div>
            ...
            <div className={`bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 transition-all ${!formActive ? "opacity-70 pointer-events-none select-none" : ""}`}>
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">Quick Tags</h3>
              <div className="flex flex-wrap gap-2">
                {TAGS.map((t) => (
            ...
            <div className={`bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 transition-all ${!formActive ? "opacity-70 pointer-events-none select-none" : ""}`}>
              <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">Share Your Thoughts</h3>
              <textarea value={comment}
                onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setComment(e.target.value)}
                placeholder="What did you enjoy? Any suggestions for improvement?"
                rows={4} readOnly={!formActive}
            ...
            {formActive ? (
              <>
                ...
                {overallRating === 0 && (
                  <p className="text-center text-xs text-gray-400 -mt-2">Please select a star rating first</p>
                )}
                {hasExistingReview && (
