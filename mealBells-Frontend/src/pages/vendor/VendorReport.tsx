...
import {
  Package, Clock, Star, Utensils, TrendingUp,
  Award, Zap, Users, Loader2,
} from 'lucide-react';
...
const DonutChart = ({ veg, nonVeg }: { veg: number; nonVeg: number }) => {
  ...
  return (
    <svg width="140" height="140" viewBox="0 0 140 140">
      ...
      <circle
        cx={cx} cy={cy} r={r}
        fill="none" stroke="#f97316" strokeWidth="18"
...
const VendorReport = () => {
  ...
  const { analyticsData, analyticsLoading, analyticsError } = useSelector(
  ...
  useEffect(() => {
    dispatch(fetchVendorAnalytics(period));
  }, [dispatch, period]);
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-6 text-center shadow-sm">
          ...
          <button
            onClick={() => dispatch(fetchVendorAnalytics(period))}
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Track your delivery performance and meal trends
          </p>
        </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl p-6 border border-gray-100">
            ...
            {chartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={chartData} barSize={period === 'year' ? 18 : 28}>
                  <XAxis
                    dataKey="day"
                    axisLine={false}
                    tickLine={false}
                  ...
                  <Bar dataKey="boxes" radius={[6, 6, 0, 0]}>
                    {chartData.map((entry, i) => (
                      <Cell
                        key={i}
                        fill={entry.boxes === maxVal ? '#f97316' : '#fed7aa'}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            {
              icon:   Package,
              label:  'Total boxes delivered',
              value:  (d?.totalBoxes ?? 0).toLocaleString(),
              sub:    `This ${period}`,
            ...
            },
            {
              ...
              value:  (d?.avgDailyMeals ?? 0).toLocaleString(),
              sub:    'Daily avg',
            ...
            },
            {
              icon:   Clock,
              label:  'Peak day',
              value:  d?.peakDay.label ?? '—',
              sub:    `${(d?.peakDay.orders ?? 0).toLocaleString()} orders`,
              accent: 'bg-purple-100 text-purple-500',
            },
            {
              icon:   Star,
              label:  'Avg rating',
              value:  d?.avgRating ? String(d.avgRating) : '—',
              sub:    `${d?.totalReviews ?? 0} reviews`,
            ...
            <div key={label} className="bg-white rounded-2xl p-5 border border-gray-100">
              ...
              <p className="text-xs font-semibold text-gray-400 flex items-center gap-0.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {d?.mostPopular ? (
                <>
                  <div className="flex items-center gap-3 mb-3">
                    {d.mostPopular.image ? (
                      <img
                        src={d.mostPopular.image}
                        alt={d.mostPopular.name}
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        Ordered {d.mostPopular.orders.toLocaleString()} times
                      </p>
                    </div>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-1.5">
                    <div
                      ...
                      style={{ width: `${d.mostPopular.percent}%` }}
                    />
                  </div>
                  <p className="text-xs text-gray-400 mt-1.5 text-right">
                    {d.mostPopular.percent}% of total
                  </p>
                </>
              ) : (
                ...
                <>
                  <div className="flex items-center gap-3 mb-3">
                    {d.leastPopular.image ? (
                      <img
                        src={d.leastPopular.image}
                        alt={d.leastPopular.name}
                        className="w-14 h-14 rounded-xl object-cover shrink-0"
                      />
                    ) : (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        Ordered {d.leastPopular.orders.toLocaleString()} times
                      </p>
                    </div>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-1.5">
                    <div
                      ...
                      style={{ width: `${d.leastPopular.percent}%` }}
                    />
                  </div>
                  <p className="text-xs text-gray-400 mt-1.5 text-right">
                    {d.leastPopular.percent}% of total
                  </p>
                </>
              ) : (
                <p className="text-sm text-gray-400">No data yet.</p>
          ...
          {/* Growth opportunity */}
          <div className="bg-gradient-to-br from-orange-500 to-orange-400 rounded-2xl p-6 flex flex-col justify-between">
            <div>
              ...
              {d?.peakDay.label && d.peakDay.label !== '—' ? (
                <p className="text-orange-100 text-sm leading-relaxed">
                  ...
                  <span className="text-white font-bold">
                    {d.peakDay.orders.toLocaleString()} orders
                  </span>
                  . Consider scheduling your most popular dishes on slower days to
                  boost overall delivery volume.
                </p>
              ) : (
                <p className="text-orange-100 text-sm leading-relaxed">
            ...
            <button className="mt-5 w-full py-2.5 bg-white text-orange-500 font-bold text-sm rounded-xl hover:bg-orange-50 transition-colors">
              View Strategy
            </button>
          </div>
        </div>
