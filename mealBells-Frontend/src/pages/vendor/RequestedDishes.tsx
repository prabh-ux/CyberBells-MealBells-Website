...
import {
  fetchVendorDishRequests,
  respondToVendorDishRequest,
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-full ${avatarColor(name)} flex items-center justify-center text-white text-sm font-semibold overflow-hidden shrink-0`}>
          {item.user?.avatar ? (
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${dietCls}`}>
              {dietLabel}
            </span>
          </div>
          <p className="text-xs text-gray-400">
            Requested {fmtDate(item.requestedDate)}
          </p>
        </div>
      </div>
      ...
      <div className="pl-0">
        ...
        {item.vendorStatus !== "pending" ? (
          <div className={`flex items-center gap-2 py-3 px-4 rounded-2xl text-sm font-semibold ${
          ...
          <div className="flex items-center gap-3 w-full">
            <button
              ...
              className="flex-[3] py-3.5 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm text-center flex items-center justify-center gap-2"
            >
              {isResponding ? (
            ...
            <button
              ...
              disabled={isResponding}
              className="flex-1 py-3.5 text-sm font-semibold text-black bg-gray-200 hover:bg-gray-300 disabled:opacity-50 rounded-lg transition-colors text-center"
            >
              Ignore
            </button>
          </div>
        ...
        <div className="flex justify-between items-start mb-5">
          <div>
            ...
            <p className="text-md text-gray-500">
              Review and respond to employee meal requests.
            </p>
          </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {(["pending", "accepted", "ignored", "all"] as const).map((s) => (
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors capitalize ${
                  statusFilter === s
